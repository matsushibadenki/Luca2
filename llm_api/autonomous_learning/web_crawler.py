...
# 役割: 自律的なWeb巡回とコンテンツ分析のコアロジックを実装する。
...
class AutonomousWebCrawler:
    ...
    def __init__(self, provider: LLMProvider, web_search_func: Any, web_fetch_func: Any):
        """
    ...
    async def _analyze_discovered_content(self, url: str) -> Optional[WebContent]:
        ...
        try:
            ...
            if not page_result or page_result.get('error') or len(page_result.get('content', '')) < 200:
                return None
