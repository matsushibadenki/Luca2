...
import logging
import asyncio 
from typing import Any, Dict, Optional, cast, List, Awaitable # Awaitableをインポート
...
# --- New Function: check_provider_health ---
...
def get_provider(
    ...
    **kwargs: Any
    ...
    Args:
        provider_name: プロバイダー名 (例: "openai", "claude")
        enhanced: V2拡張プロバイダーを取得するかどうか
        **kwargs: プロバイダーの初期化に渡す追加の引数 (例: n_gpu_layers, model_path)

    Returns:
        LLMProviderまたはEnhancedLLMProviderのインスタンス
    """
    ...
    cache_key = f"{base_provider_name}{cache_key_suffix}"
    init_kwargs_for_provider_instance = kwargs.copy()
    if base_provider_name == 'ollama' or base_provider_name == 'huggingface' or base_provider_name == 'gemini' or base_provider_name == 'claude' or base_provider_name == 'openai':
        init_kwargs_for_provider_instance.pop('model', None)
    
    if 'n_gpu_layers' in kwargs and kwargs['n_gpu_layers'] is not None:
         cache_key += f"_gpu:{kwargs['n_gpu_layers']}"
    if 'model_path' in kwargs:
        cache_key += f"_path:{kwargs['model_path']}"
    ...
    try:
        if enhanced:
            standard_provider_kwargs = {k: v for k, v in kwargs.items() if k in ['model', 'model_path', 'n_gpu_layers']}
            standard_provider = get_provider(base_provider_name, enhanced=False, **standard_provider_kwargs)
            instance = provider_class(standard_provider=standard_provider, **init_kwargs_for_provider_instance)
        else:
            ...
            elif base_provider_name == 'ollama':
                instance = provider_class()
            elif base_provider_name == 'huggingface':
                instance = provider_class()
            elif base_provider_name == 'gemini':
                instance = provider_class()
            elif base_provider_name == 'claude':
                instance = provider_class()
            elif base_provider_name == 'openai':
                instance = provider_class()
            else:
                instance = provider_class(**init_kwargs_for_provider_instance)
